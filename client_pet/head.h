...
class Pet{
    ...
    public:
        ...
        const void Qshowname(char*);    /*图形界面函数：获取精灵名称*/
        const void Qshowskname(char*);  /*图形界面函数：获取精灵技能名称*/
};
...
const void Pet::Qshowname(char* text){
    memcpy(text, name, 20);
}

const void Pet::Qshowskname(char * text){
    memcpy(text, skill_name, 20);
}
...
class Battle{/*此类及其成员在图形界面版本中不使用*/
    public:
        Pet userpet;
        Pet syspet;
        int userhp;
...
Battle::Battle(int usr_id, int usr_lv, int sys_id, int sys_lv){
